...
def _run_ocr_on_image(reader, image_path: str, output_dir: str) -> dict:
    """
    OCR a single image, extract invoice fields, and write the raw text to disk.

    Returns a populated result dict on success, or one with status='error' on failure.
    """
...
def process_image_cpu(image_path: str, output_dir: str) -> dict:
    """
    Multiprocessing worker entry point for CPU mode.

    Each subprocess must create its own PaddleOCR reader since the object
